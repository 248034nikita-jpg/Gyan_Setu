...
-- ============================================================================
-- FINAL SETUP v2 — gyansetudb
-- topic/concept are now NOT NULL DEFAULT '' (empty string = "not applicable"),
-- instead of NULL. Simpler comparisons in PHP (plain =, no NULL-safe <=> needed).
-- ============================================================================

USE `gyansetudb`;

-- STEP 1: scores — add topic/concept + total_questions
ALTER TABLE `scores`
    ADD COLUMN `topic` VARCHAR(50) NOT NULL DEFAULT '' AFTER `difficulty_tier_played`,
    ADD COLUMN `concept` VARCHAR(100) NOT NULL DEFAULT '' AFTER `topic`,
    ADD COLUMN `total_questions` INT NOT NULL DEFAULT 20 AFTER `score_value`;

-- STEP 2: views — auto-calculated from scores, nothing to insert manually
CREATE OR REPLACE VIEW `view_child_daily_activity` AS
SELECT child_id, game_id, DATE(date_played) AS activity_date,
       COUNT(*) AS rounds_played, SUM(coins_earned) AS coins_earned_today
FROM `scores`
GROUP BY child_id, game_id, DATE(date_played);

CREATE OR REPLACE VIEW `view_child_best_streak` AS
SELECT child_id, game_id, MAX(streak_achieved) AS best_streak
FROM `scores`
GROUP BY child_id, game_id;

CREATE OR REPLACE VIEW `view_child_game_progress` AS
SELECT child_id, game_id, difficulty_tier_played AS difficulty_tier, topic, concept,
       COUNT(*) AS attempts, MAX(score_value) AS best_score, MAX(accuracy_percentage) AS best_accuracy
FROM `scores`
GROUP BY child_id, game_id, difficulty_tier_played, topic, concept;

-- STEP 3a: criteria_types — lookup table replacing the old ENUM
...
INSERT INTO `criteria_types` (`type_name`, `description`) VALUES
('rounds_completed', 'Total attempts across matching rounds meets threshold_value'),
('perfect_score', 'A single round scored at or above threshold_value'),
('streak', 'Best-ever correct-answer streak meets threshold_value'),
('accuracy_threshold', 'A single round''s accuracy meets threshold_value (%)'),
('topic_all_tiers', 'All difficulty tiers within a topic have been attempted'),
('game_all_rounds', 'All round-types within a game have been attempted'),
('daily_streak', 'Played on threshold_value consecutive days');

-- STEP 3b: badge_criteria (criteria_type is now a FK, not an ENUM)
CREATE TABLE IF NOT EXISTS `badge_criteria` (
    `criteria_id` INT AUTO_INCREMENT PRIMARY KEY,
    `badge_id` INT NOT NULL,
    `criteria_type_id` INT NOT NULL,
    `game_id` INT DEFAULT NULL,              -- NULL = any game
    `topic` VARCHAR(50) NOT NULL DEFAULT '', -- '' = any topic
    `concept` VARCHAR(100) NOT NULL DEFAULT '', -- '' = any concept
    `difficulty_tier` INT DEFAULT NULL,      -- NULL = any tier
    `threshold_value` INT NOT NULL,
    CONSTRAINT `fk_criteria_badge` FOREIGN KEY (`badge_id`) REFERENCES `badges` (`badge_id`)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_criteria_game` FOREIGN KEY (`game_id`) REFERENCES `games` (`game_id`)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_criteria_type` FOREIGN KEY (`criteria_type_id`) REFERENCES `criteria_types` (`criteria_type_id`)
        ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- STEP 4: the 10 badges
INSERT INTO `badges` (`title`, `description`, `coins_reward`) VALUES
('First Steps', 'Complete your first round of any topic.', 5),
('Grammar Starter', 'Finish the is / am / are round.', 5),
('Word Explorer', 'Finish the opposites round.', 5),
('Grammar Master', 'Complete all 3 grammar tiers.', 15),
('Vocabulary Master', 'Complete all 3 vocabulary tiers.', 15),
('English Champion', 'Complete all 6 rounds across both grammar and vocabulary.', 25),
('Perfect Score', 'Get 20 out of 20 correct in any single round.', 10),
('Sharp Shooter', 'Answer 5 questions correctly in a row without a miss.', 10),
('Hard Mode Hero', 'Score 90% or higher on any Hard-tier round.', 15),
('Weekly Whacker', 'Play at least once every day for 7 days in a row.', 20);

-- STEP 5: link badges to criteria (looks up criteria_type_id by name)
INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, NULL, '', '', NULL, 1
FROM `badges` b, `criteria_types` ct WHERE b.title = 'First Steps' AND ct.type_name = 'rounds_completed';

INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, 1, 'grammar', 'is / am / are', 1, 1
FROM `badges` b, `criteria_types` ct WHERE b.title = 'Grammar Starter' AND ct.type_name = 'rounds_completed';

INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, 1, 'vocabulary', 'opposites', 1, 1
FROM `badges` b, `criteria_types` ct WHERE b.title = 'Word Explorer' AND ct.type_name = 'rounds_completed';

INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, 1, 'grammar', '', NULL, 3
FROM `badges` b, `criteria_types` ct WHERE b.title = 'Grammar Master' AND ct.type_name = 'topic_all_tiers';

INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, 1, 'vocabulary', '', NULL, 3
FROM `badges` b, `criteria_types` ct WHERE b.title = 'Vocabulary Master' AND ct.type_name = 'topic_all_tiers';

INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, 1, '', '', NULL, 6
FROM `badges` b, `criteria_types` ct WHERE b.title = 'English Champion' AND ct.type_name = 'game_all_rounds';

INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, NULL, '', '', NULL, 20
FROM `badges` b, `criteria_types` ct WHERE b.title = 'Perfect Score' AND ct.type_name = 'perfect_score';

INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, NULL, '', '', NULL, 5
FROM `badges` b, `criteria_types` ct WHERE b.title = 'Sharp Shooter' AND ct.type_name = 'streak';

INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, NULL, '', '', 3, 90
FROM `badges` b, `criteria_types` ct WHERE b.title = 'Hard Mode Hero' AND ct.type_name = 'accuracy_threshold';

INSERT INTO `badge_criteria` (`badge_id`, `criteria_type_id`, `game_id`, `topic`, `concept`, `difficulty_tier`, `threshold_value`)
SELECT b.badge_id, ct.criteria_type_id, NULL, '', '', NULL, 7
FROM `badges` b, `criteria_types` ct WHERE b.title = 'Weekly Whacker' AND ct.type_name = 'daily_streak';
