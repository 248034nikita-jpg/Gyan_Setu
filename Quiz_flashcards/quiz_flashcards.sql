ALTER TABLE children ADD COLUMN total_stars INT NOT NULL DEFAULT 0 AFTER total_coins;

INSERT INTO games (title, slug, game_type, description, min_age, max_age, is_active) 
VALUES (
    'Quiz & Flashcards',
    'quiz-flashcards',
    'quiz_flashcards',
    'Science, Nature, Space quiz and flashcards with fun facts and puzzles. Learn about Nepal, animals, planets, and more!',
    4, 12, 1
);
...
CREATE TABLE IF NOT EXISTS quiz_levels (
    ...
    icon VARCHAR(10),
    difficulty_tier INT NOT NULL, -- 1=basic, 2=intermediate, 3=advanced
    description_en TEXT,
    description_ne TEXT,
    is_active TINYINT(1) DEFAULT 1,
    FOREIGN KEY (subject_id) REFERENCES quiz_subjects(subject_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS quiz_questions (
    ...
    fun_fact_en TEXT,
    fun_fact_ne TEXT,
    correct_order JSON NULL, -- for puzzle: array of item id codes
    FOREIGN KEY (level_id) REFERENCES quiz_levels(level_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS puzzle_items (
    ...
    question_id INT NOT NULL,
    item_id_code VARCHAR(10) NOT NULL, -- e.g., 'a', 'b', 'c'
...
CREATE TABLE IF NOT EXISTS flashcard_cards (
    ...
    tag_en VARCHAR(50),
    tag_ne VARCHAR(50),
    facts_en JSON NOT NULL, -- array of fact strings
    facts_ne JSON NOT NULL,
    FOREIGN KEY (deck_id) REFERENCES flashcard_decks(deck_id) ON DELETE CASCADE
);

-- Child progress tables (for tracking)
CREATE TABLE IF NOT EXISTS child_question_progress (
...
CREATE TABLE IF NOT EXISTS child_flashcard_progress (
    child_id INT NOT NULL,
...
CREATE INDEX idx_quiz_levels_subject ON quiz_levels(subject_id);
CREATE INDEX idx_quiz_questions_level ON quiz_questions(level_id);
CREATE INDEX idx_quiz_options_question ON quiz_options(question_id);
CREATE INDEX idx_puzzle_items_question ON puzzle_items(question_id);
...
-- 1. Subjects
INSERT INTO quiz_subjects (subject_id, name_en, name_ne, icon, description_en, description_ne) VALUES
...
-- 2. Levels
INSERT INTO quiz_levels (level_id, subject_id, level_name_en, level_name_ne, icon, difficulty_tier, description_en, description_ne) VALUES
...
-- 3. Questions and options/puzzle items
-- Science Basic (level_id=1)

-- Question 1001: MCQ
INSERT INTO quiz_questions (question_id, level_id, type, question_en, question_ne, fun_fact_en, fun_fact_ne) VALUES
...
INSERT INTO quiz_options (option_id, question_id, option_text_en, option_text_ne, emoji, is_correct) VALUES
...
-- Question 1006: Puzzle
INSERT INTO quiz_questions (question_id, level_id, type, question_en, question_ne, hint_en, hint_ne, fun_fact_en, fun_fact_ne, correct_order) VALUES
...
INSERT INTO puzzle_items (item_id, question_id, item_id_code, item_label_en, item_label_ne, emoji) VALUES
(3001, 1006, 'a', 'Solid', 'ठोस', '🧊'),
(3002, 1006, 'b', 'Liquid', 'तरल', '💧'),
(3003, 1006, 'c', 'Gas', 'ग्यास', '♨️');

-- Science Intermediate (level_id=2)
...
-- Nature Intermediate (level_id=5)
...
-- Insert decks for flashcard levels (level_id 10,11,12)
...
(4001, 1, '☀️', 'The Sun', 'सूर्य', 'Our Star', 'हाम्रो तारा', 'Star', 'तारा', 
'["The Sun gives us light and heat – it\'s a big, bright star!","It\'s so huge that 1 million Earths could fit inside!","The Sun is about 5,500°C on the surface – that\'s super hot!"]',
'["सूर्यले हामीलाई उज्यालो र गर्मी दिन्छ – यो एउटा ठूलो, चम्किलो तारा हो!","यति ठूलो छ कि १० लाख पृथ्वीहरू यसको भित्र अटाउन सक्छन्!","सूर्यको सतहको तापक्रम करिब ५,५००°C छ – त्यो धेरै तातो हो!"]'),

(4002, 1, '☿', 'Mercury', 'बुध', 'The Swift Planet', 'छिटो ग्रह', 'Terrestrial', 'स्थलीय',
'["Mercury is the tiniest planet and the closest to the Sun!","It has almost no air, so days are boiling hot and nights are freezing cold!","A year on Mercury is just 88 Earth days – super short!"]',
'["बुध सबैभन्दा सानो ग्रह हो र सूर्यको सबैभन्दा नजिक छ!","यहाँ लगभग हावा छैन, त्यसैले दिनमा धेरै तातो र रातमा धेरै चिसो हुन्छ!","बुधको एक वर्ष जम्मा ८८ पृथ्वी दिन हो – धेरै छोटो!"]'),

(4003, 1, '♀', 'Venus', 'शुक्र', 'The Evening Star', 'साँझको तारा', 'Terrestrial', 'स्थलीय',
'["Venus is the hottest planet – even hotter than Mercury!","It spins backwards – the Sun rises in the west and sets in the east!","Its thick clouds are made of acid – yikes!"]',
'["शुक्र सबैभन्दा तातो ग्रह हो – बुधभन्दा पनि तातो!","यो उल्टो घुम्छ – सूर्य पश्चिममा उदाउँछ र पूर्वमा अस्ताउँछ!","यसको बाक्लो बादलहरू एसिडले बनेका छन् – ओहो!"]'),

(4004, 1, '🌍', 'Earth', 'पृथ्वी', 'The Blue Marble', 'निलो संगमरमर', 'Terrestrial', 'स्थलीय',
'["Earth is the only planet we know that has life!","About 71% of Earth is covered with water – that\'s why it looks blue!","We have one moon that lights up our night sky."]',
'["पृथ्वी मात्र त्यो ग्रह हो जहाँ जीवन छ!","पृथ्वीको करिब ७१% पानीले ढाकिएको छ – त्यसैले यो निलो देखिन्छ!","हामीसँग एउटा चन्द्रमा छ जसले रातको आकाश उज्यालो बनाउँछ।"]'),

(4005, 1, '♂', 'Mars', 'मंगल', 'The Red Planet', 'रातो ग्रह', 'Terrestrial', 'स्थलीय',
'["Mars is called the Red Planet because of rusty iron on its surface!","It has the tallest volcano in the solar system – Olympus Mons!","Mars has two tiny moons: Phobos and Deimos."]',
'["मंगललाई रातो ग्रह भनिन्छ किनभने यसको सतहमा खिया लागेको फलाम छ!","यसमा सौर्यमण्डलको सबैभन्दा अग्लो ज्वालामुखी छ – ओलम्पस मोन्स!","मंगलका दुई साना चन्द्रमा छन्: फोबोस र डेमोस।"]'),

(4006, 1, '🌙', 'The Moon', 'चन्द्रमा', 'Earth\'s Companion', 'पृथ्वीको साथी', 'Moon', 'चन्द्रमा',
'["The Moon is Earth\'s only natural satellite – it orbits around us!","It\'s about 384,400 km away – that\'s far but we can see it every night!","There\'s no air on the Moon, so footprints stay forever!"]',
'["चन्द्रमा पृथ्वीको एकमात्र प्राकृतिक उपग्रह हो – यो हाम्रो वरिपरि घुम्छ!","यो करिब ३,८४,४०० किमी टाढा छ – त्यो धेरै टाढा तर हामी यसलाई हरेक रात देख्न सक्छौं!","चन्द्रमामा हावा छैन, त्यसैले पाइलाका छाप सधैं रहन्छन्!"]'),

(4007, 1, '🧊', 'States of Matter', 'पदार्थका अवस्थाहरू', 'Solid, Liquid, Gas', 'ठोस, तरल, ग्यास', 'Science', 'विज्ञान',
'["Solids keep their shape – like ice cubes and rocks!","Liquids flow and take the shape of their container – like water and milk!","Gases fill up any space – like the air we breathe!","Heating or cooling can change matter from one state to another – cool, right?"]',
'["ठोस पदार्थहरू आफ्नो आकार राख्छन् – जस्तै बरफका टुक्रा र ढुङ्गा!","तरल पदार्थ बग्छन् र भाँडाको आकार लिन्छन् – जस्तै पानी र दूध!","ग्यासहरू कुनै पनि ठाउँ भर्छन् – जस्तै हामीले सास फेर्ने हावा!","तातो वा चिसोले पदार्थको अवस्था परिवर्तन गर्न सक्छ – रमाइलो, हैन?"]'),

(4008, 1, '💧', 'The Water Cycle', 'पानीको चक्र', 'Water on the Move', 'पानीको गतिशीलता', 'Nature', 'प्रकृति',
'["Evaporation: the Sun heats water and turns it into vapor!","Condensation: vapor cools and makes clouds!","Precipitation: water falls back as rain or snow!","Collection: water gathers in rivers, lakes, and oceans – and the cycle starts again!"]',
'["वाष्पीकरण: सूर्यले पानीलाई तताउँछ र वाष्प बनाउँछ!","संघनन: वाष्प चिसो हुन्छ र बादल बनाउँछ!","वर्षा: पानी फेरि वर्षा वा हिउँको रूपमा खस्छ!","सङ्कलन: पानी नदी, ताल र महासागरमा जम्मा हुन्छ – र चक्र फेरि सुरु हुन्छ!"]'),

(4009, 1, '🌱', 'Plant Parts', 'बोटका भागहरू', 'Roots, Stem, Leaves, Flowers', 'जरा, डाँठ, पात, फूल', 'Nature', 'प्रकृति',
'["Roots hold the plant in the ground and drink water and minerals!","Stems stand tall and carry food and water around the plant!","Leaves are like little kitchens – they make food using sunlight!","Flowers are the pretty part – they make seeds for new plants!"]',
'["जराले बोटलाई जमिनमा टाँस्छ र पानी र खनिज पिउँछ!","डाँठले बोटलाई उभ्याउँछ र खाना र पानी बोटभरि पुर्याउँछ!","पातहरू सानो भान्सा जस्तै हुन् – तिनीहरूले सूर्यको प्रकाश प्रयोग गरेर खाना बनाउँछन्!","फूलहरू सुन्दर भाग हुन् – तिनीहरूले नयाँ बोटका लागि बीउ बनाउँछन्!"]'),

(4010, 1, '👁️', 'The Five Senses', 'पाँच इन्द्रियहरू', 'See, Hear, Smell, Taste, Touch', 'देख्नु, सुन्नु, सुँघ्नु, स्वाद लिनु, छुनु', 'Science', 'विज्ञान',
'["Sight – our eyes help us see colors, shapes, and faces!","Hearing – our ears catch sounds and music!","Smell – our nose picks up yummy and yucky smells!","Taste – our tongue tastes sweet, sour, salty, bitter, and umami!","Touch – our skin feels things like soft, rough, hot, and cold!"]',
'["दृष्टि – हाम्रा आँखाले रंग, आकार र अनुहार देख्न मद्दत गर्छ!","श्रवण – हाम्रा कानले आवाज र संगीत सुन्छन्!","घ्राण – हाम्रो नाकले मीठो र फोहोर गन्ध पत्ता लगाउँछ!","स्वाद – हाम्रो जिब्रोले मीठो, अमिलो, नुनिलो, तीतो र उमामी स्वाद लिन्छ!","स्पर्श – हाम्रो छालाले नरम, नराम्रो, तातो र चिसो महसुस गर्छ!"]'),

(4011, 1, '☀️', 'Weather Types', 'मौसम प्रकार', 'Sunny, Rainy, Cloudy, Snowy', 'घाम, वर्षा, बादली, हिउँ', 'Nature', 'प्रकृति',
'["Sunny – bright and warm, great for playing outside!","Rainy – water drops fall from clouds, don\'t forget your umbrella!","Cloudy – the sky is covered with clouds, sometimes it rains!","Snowy – ice crystals fall as snowflakes, perfect for snowmen!","Weather changes how we dress and what we do every day!"]',
'["घाम – उज्यालो र न्यानो, बाहिर खेल्नको लागि उत्तम!","वर्षा – बादलबाट पानीका थोपा खस्छन्, आफ्नो छाता नबिर्सनुहोस्!","बादली – आकाश बादलले ढाकिएको छ, कहिलेकाहीँ वर्षा हुन्छ!","हिउँ – बरफका क्रिस्टल हिउँका फोका जस्तै खस्छन्, हिउँमानिस बनाउन उत्तम!","मौसमले हाम्रो पहिरन र हामीले दैनिक के गर्छौं भनेर परिवर्तन गर्छ!"]'),

(4012, 1, '🌿', 'Living vs Non-Living', 'जीवित र निर्जीव', 'What is alive?', 'के जीवित छ?', 'Science', 'विज्ञान',
'["Living things grow, need food, and can have babies – like plants and animals!","People, dogs, and trees are living things.","Non-living things don\'t grow or need food – like rocks, chairs, and toys!","Living things react to changes around them – like a plant bending toward the sun!"]',
'["जीवित चीजहरू बढ्छन्, खाना चाहिन्छ र बच्चा जन्माउँछन् – जस्तै बोट र जनावर!","मानिस, कुकुर र रूखहरू जीवित चीजहरू हुन्।","निर्जीव चीजहरू बढ्दैनन् र खाना चाहिँदैन – जस्तै ढुङ्गा, कुर्सी र खेलौना!","जीवित चीजहरू आफ्नो वरपरको परिवर्तनमा प्रतिक्रिया दिन्छन् – जस्तै सूर्यतिर झुकेको बोट!"]');

-- Intermediate flashcards (deck_id=2)
INSERT INTO flashcard_cards (card_id, deck_id, card_icon, name_en, name_ne, subtitle_en, subtitle_ne, tag_en, tag_ne, facts_en, facts_ne) VALUES
(4013, 2, '♃', 'Jupiter', 'बृहस्पति', 'The Giant', 'विशाल', 'Gas Giant', 'ग्यासको विशाल',
'["Jupiter is the biggest planet – you could fit 1,300 Earths inside!","It has a giant storm called the Great Red Spot – it\'s bigger than Earth!","Jupiter has 79 known moons – that\'s a lot of friends!"]',
'["बृहस्पति सबैभन्दा ठूलो ग्रह हो – यसको भित्र १३०० पृथ्वीहरू अटाउन सक्छन्!","यसमा ठूलो आँधी छ जसलाई ठूलो रातो धब्बा भनिन्छ – यो पृथ्वीभन्दा ठूलो छ!","बृहस्पतिसँग ७९ ज्ञात चन्द्रमा छन् – त्यो धेरै साथीहरू!"]'),

(4014, 2, '♄', 'Saturn', 'शनि', 'The Ringed World', 'वलय भएको संसार', 'Gas Giant', 'ग्यासको विशाल',
'["Saturn has beautiful rings made of ice and rock – they sparkle!","Saturn is so light that it would float in a giant bathtub – weird, right?","It has 82 known moons – the most in the solar system!"]',
'["शनिसँग बरफ र ढुङ्गाले बनेका सुन्दर वलय छन् – तिनीहरू चम्किन्छन्!","शनि यति हलुका छ कि यो एउटा विशाल नुहाउने टबमा पौडिन सक्छ – अनौठो, हैन?","यससँग ८२ ज्ञात चन्द्रमा छन् – सौर्यमण्डलमा सबैभन्दा धेरै!"]'),

(4015, 2, '⛢', 'Uranus', 'युरेनस', 'The Sideways Planet', 'छेउको ग्रह', 'Ice Giant', 'बरफको विशाल',
'["Uranus spins on its side – almost like a rolling ball!","It\'s the coldest planet – -224°C, brrr!","It has 27 known moons, all named after Shakespeare characters!"]',
'["युरेनस आफ्नो छेउमा घुम्छ – लगभग एउटा बल जस्तै!","यो सबैभन्दा चिसो ग्रह हो – -२२४°C, हिउँ!","यससँग २७ ज्ञात चन्द्रमा छन्, सबै शेक्सपियरका पात्रहरूको नाममा!"]'),

(4016, 2, '♆', 'Neptune', 'नेप्च्युन', 'The Windy World', 'हावायुक्त संसार', 'Ice Giant', 'बरफको विशाल',
'["Neptune is the windiest planet – winds blow at 2,100 km/h!","It\'s the farthest planet from the Sun – very cold and dark!","It has 14 known moons, with the largest named Triton."]',
'["नेप्च्युन सबैभन्दा हावायुक्त ग्रह हो – हावा २,१०० किमी/घण्टाको गतिले बहन्छ!","यो सूर्यबाट सबैभन्दा टाढाको ग्रह हो – धेरै चिसो र अँधेरो!","यससँग १४ ज्ञात चन्द्रमा छन्, सबैभन्दा ठूलोको नाम ट्राइटन हो।"]'),

(4017, 2, '☄️', 'Asteroid Belt', 'क्षुद्रग्रह घेरा', 'Between Mars & Jupiter', 'मंगल र बृहस्पतिबीच', 'Belt', 'घेरा',
'["The asteroid belt is a huge ring of rocks between Mars and Jupiter!","There are millions of asteroids – some are tiny, some are huge!","The biggest one is Ceres – it\'s a dwarf planet!"]',
'["क्षुद्रग्रह घेरा मंगल र बृहस्पतिबीच ढुङ्गाहरूको विशाल घेरा हो!","त्यहाँ लाखौं क्षुद्रग्रहहरू छन् – कोही साना, कोही ठूला!","सबैभन्दा ठूलो सेरेस हो – यो एउटा बौना ग्रह हो!"]'),

(4018, 2, '🪐', 'Pluto', 'प्लुटो', 'Dwarf Planet', 'बौना ग्रह', 'Dwarf Planet', 'बौना ग्रह',
'["Pluto is a dwarf planet – it\'s smaller than our Moon!","It has 5 moons – the biggest one is Charon, almost as big as Pluto!","Pluto was reclassified as a dwarf planet in 2006 – poor Pluto!"]',
'["प्लुटो एउटा बौना ग्रह हो – यो हाम्रो चन्द्रमाभन्दा सानो छ!","यससँग ५ चन्द्रमा छन् – सबैभन्दा ठूलो चारोन हो, लगभग प्लुटो जति ठूलो!","प्लुटोलाई २००६ मा बौना ग्रहको रूपमा पुन: वर्गीकृत गरियो – गरिब प्लुटो!"]'),

(4019, 2, '🌱', 'Photosynthesis', 'प्रकाश संश्लेषण', 'How Plants Make Food', 'बोटबिरुवाले कसरी खाना बनाउँछन्', 'Science', 'विज्ञान',
'["Plants use sunlight, water, and CO₂ to make their own food!","The green stuff in leaves – chlorophyll – catches sunlight!","Plants release oxygen – that\'s the air we breathe!","Without plants, we wouldn\'t have any oxygen – thank you, plants!"]',
'["बोटबिरुवाले आफ्नो खाना बनाउन सूर्यको प्रकाश, पानी र CO₂ प्रयोग गर्छन्!","पातको हरियो भाग – क्लोरोफिल – सूर्यको प्रकाश समात्छ!","बोटबिरुवाले अक्सिजन छोड्छन् – त्यो हामीले सास फेर्ने हावा हो!","बोटबिरुवा नभएको भए हामीसँग अक्सिजन हुँदैन – धन्यवाद, बोटबिरुवा!"]'),

(4020, 2, '🐾', 'Food Chains', 'खाद्य शृंखला', 'Who Eats Whom?', 'कसले कसलाई खान्छ?', 'Nature', 'प्रकृति',
'["Producers – like plants – make their own food from sunlight!","Consumers – like rabbits and foxes – eat other living things!","Decomposers – like fungi and bacteria – break down dead things!","A food chain shows how energy flows in nature – it\'s all connected!"]',
'["उत्पादकहरू – जस्तै बोट – सूर्यको प्रकाशबाट आफ्नो खाना बनाउँछन्!","उपभोक्ताहरू – जस्तै खरायो र फ्याक्स – अरू जीवित चीजहरू खान्छन्!","विघटनकर्ताहरू – जस्तै फङ्गस र ब्याक्टेरिया – मरेका चीजहरू विघटन गर्छन्!","खाद्य शृंखलाले प्रकृतिमा ऊर्जा कसरी प्रवाह हुन्छ देखाउँछ – यो सबै जोडिएको छ!"]'),

(4021, 2, '🧠', 'Human Body Systems', 'मानव शरीर प्रणाली', 'How We Work', 'हामी कसरी काम गर्छौं', 'Science', 'विज्ञान',
'["Skeletal system – bones support us and protect our insides!","Muscular system – muscles let us run, jump, and play!","Digestive system – it breaks down food so we get energy!","Respiratory system – our lungs take in oxygen and let out CO₂!","All systems work together – teamwork makes the body work!"]',
...
(4022, 2, '🌍', 'Ecosystems', 'पारिस्थितिकी तंत्र', 'Nature\'s Communities', 'प्रकृतिका समुदायहरू', 'Nature', 'प्रकृति',
'["An ecosystem is a community of living and non-living things!","Forests, deserts, oceans, and grasslands are all ecosystems!","Each ecosystem has its own plants, animals, and weather!","Living things depend on each other – it\'s like a big family!"]',
'["पारिस्थितिकी तंत्र भनेको जीवित र निर्जीव चीजहरूको समुदाय हो!","वन, मरुभूमि, महासागर र घाँसे मैदानहरू सबै पारिस्थितिकी तंत्र हुन्!","प्रत्येक पारिस्थितिकी तंत्रको आफ्नै बोटबिरुवा, जनावर र मौसम हुन्छ!","जीवित चीजहरू एकअर्कामा निर्भर हुन्छन् – यो ठूलो परिवार जस्तै हो!"]'),

(4023, 2, '🐾', 'Animal Classification', 'जनावर वर्गीकरण', 'Mammals, Birds, Fish & More', 'स्तनधारी, चरा, माछा र अरू', 'Nature', 'प्रकृति',
'["Mammals have fur or hair and feed milk to their babies – like lions and dogs!","Birds have feathers, lay eggs, and many can fly – like eagles and penguins!","Reptiles have scales and are cold-blooded – like snakes and turtles!","Amphibians live on land and water – like frogs and salamanders!","Fish live in water, have gills and fins – like sharks and salmon!"]',
...
(4024, 2, '🌱', 'Plant Life Cycle', 'बोटको जीवन चक्र', 'From Seed to Flower', 'बीउदेखि फूलसम्म', 'Nature', 'प्रकृति',
'["Seed – it\'s the start of a new plant, waiting to grow!","Germination – the seed sprouts and sends roots down!","Growth – the plant grows leaves and stems to catch sunlight!","Reproduction – the plant makes flowers and new seeds!","The cycle goes on as seeds spread to make new plants – nature\'s magic!"]',
'["बीउ – यो नयाँ बोटको सुरुवात हो, बढ्नको लागि पर्खिरहेको!","अंकुरण – बीउ अंकुरिन्छ र जरा तल पठाउँछ!","वृद्धि – बोटले सूर्यको प्रकाश समात्न पात र डाँठ उमार्छ!","प्रजनन – बोटले फूल र नयाँ बीउ बनाउँछ!","चक्र जारी रहन्छ किनभने बीउहरू फैलिएर नयाँ बोट बनाउँछन् – प्रकृतिको जादू!"]');

-- Advanced flashcards (deck_id=3)
INSERT INTO flashcard_cards (card_id, deck_id, card_icon, name_en, name_ne, subtitle_en, subtitle_ne, tag_en, tag_ne, facts_en, facts_ne) VALUES
(4025, 3, '☀️', 'Solar Mass', 'सौर्य द्रव्यमान', '1.989 × 10³⁰ kg', '१.९८९ × १०³⁰ किग्रा', 'Star', 'तारा',
'["The Sun is so heavy that it holds 99.86% of all the mass in our solar system!","It would take 333,000 Earths to match the Sun\'s weight – wow!","Every second, the Sun loses 4 million tons of mass as it shines – that\'s crazy!"]',
'["सूर्य यति भारी छ कि यसले हाम्रो सौर्यमण्डलको ९९.८६% द्रव्यमान समाउँछ!","सूर्यको तौल बराबर गर्न ३,३३,००० पृथ्वीहरू चाहिन्छ – वाह!","प्रत्येक सेकेन्ड, सूर्यले चम्किरहेको बेला ४ लाख टन द्रव्यमान गुमाउँछ – त्यो अचम्मको हो!"]'),

(4026, 3, '☿', 'Mercury\'s Orbit', 'बुधको कक्षा', '88 Days, Eccentric', '८८ दिन, अण्डाकार', 'Terrestrial', 'स्थलीय',
'["Mercury has the most oval-shaped orbit of all planets – it\'s not a perfect circle!","Its distance from the Sun changes a lot – from 46 to 70 million km!","Mercury zips around the Sun at 47.87 km/s – faster than any other planet!"]',
'["बुधको कक्षा सबैभन्दा अण्डाकार छ – यो पूरा वृत्त होइन!","सूर्यबाट यसको दूरी धेरै परिवर्तन हुन्छ – ४६ देखि ७० मिलियन किमी!","बुध सूर्यको वरिपरि ४७.८७ किमी/सेकेन्डको गतिमा घुम्छ – अरू कुनै ग्रहभन्दा छिटो!"]'),

(4027, 3, '♀', 'Venus\' Rotation', 'शुक्रको घुमाव', '243 Days — Longer than Year', '२४३ दिन — वर्षभन्दा लामो', 'Terrestrial', 'स्थलीय',
'["Venus takes 243 Earth days to spin once – that\'s longer than its year (225 days)!","So a day on Venus is longer than a year – weird!","It spins backwards, so the Sun rises in the west – that\'s topsy-turvy!"]',
'["शुक्रलाई एक पटक घुम्न २४३ पृथ्वी दिन लाग्छ – त्यो यसको वर्ष (२२५ दिन) भन्दा लामो छ!","त्यसैले शुक्रमा एउटा दिन वर्षभन्दा लामो हुन्छ – अनौठो!","यो उल्टो घुम्छ, त्यसैले सूर्य पश्चिममा उदाउँछ – त्यो उल्टो हुन्छ!"]'),

(4028, 3, '🌍', 'Earth\'s Axial Tilt', 'पृथ्वीको अक्षीय झुकाव', '23.5° — Causes Seasons', '२३.५° — ऋतुहरूको कारण', 'Terrestrial', 'स्थलीय',
'["Earth is tilted at 23.5° – that\'s why we have four seasons!","The tilt changes slightly over 41,000 years – a slow wobble!","The tilt also makes days longer in summer and shorter in winter!"]',
'["पृथ्वी २३.५° मा झुकेको छ – त्यसैले हामीलाई चार ऋतुहरू हुन्छन्!","झुकाव ४१,००० वर्षमा अलि परिवर्तन हुन्छ – एक ढिलो हल्लाउने!","झुकावले गर्मीमा दिन लामो र जाडोमा छोटो बनाउँछ!"]'),

(4029, 3, '♂', 'Mars\' Atmosphere', 'मंगलको वायुमण्डल', '95% CO₂, Very Thin', '९५% CO₂, धेरै पातलो', 'Terrestrial', 'स्थलीय',
'["Mars\' atmosphere is very thin – only 1% as thick as Earth\'s!","It\'s mostly carbon dioxide (95%) – not good for breathing!","Dust storms on Mars can cover the whole planet – like a big blanket!"]',
'["मंगलको वायुमण्डल धेरै पातलो छ – पृथ्वीको भन्दा १% मात्र बाक्लो!","यो प्रायः कार्बन डाइअक्साइड (९५%) हो – सास फेर्नको लागि राम्रो छैन!","मंगलमा धुलोको आँधीले पूरै ग्रह ढाक्न सक्छ – ठूलो कम्बल जस्तै!"]'),

(4030, 3, '♃', 'Jupiter\'s Composition', 'बृहस्पतिको संरचना', '90% H, 10% He', '९०% H, १०% He', 'Gas Giant', 'ग्यासको विशाल',
'["Jupiter is mostly hydrogen (90%) and helium (10%) – just like the Sun!","It has no solid surface – it\'s a giant ball of gas!","Jupiter\'s gravity is 2.5 times stronger than Earth\'s – you\'d be heavier there!"]',
'["बृहस्पति प्रायः हाइड्रोजन (९०%) र हेलियम (१०%) हो – सूर्य जस्तै!","यसको कुनै ठोस सतह छैन – यो ग्यासको विशाल बल हो!","बृहस्पतिको गुरुत्वाकर्षण पृथ्वीको भन्दा २.५ गुणा बलियो छ – त्यहाँ तपाईं भारी हुनुहुन्छ!"]'),

(4031, 3, '🧬', 'DNA & Genetics', 'DNA र आनुवंशिकता', 'The Blueprint of Life', 'जीवनको नक्सा', 'Science', 'विज्ञान',
'["DNA is like a recipe book that tells living things how to grow and function!","Genes are small bits of DNA that decide traits like eye color and height!","We get our DNA from our parents – that\'s why we look like them!","Changes in DNA – mutations – can create new traits and help evolution!"]',
'["DNA एउटा रेसिपी किताब जस्तै हो जसले जीवित चीजहरूलाई कसरी बढ्न र काम गर्ने भन्ने बताउँछ!","जीनहरू DNA का साना टुक्रा हुन् जसले आँखाको रङ र उचाइ जस्ता विशेषताहरू निर्धारण गर्छन्!","हामी आफ्नो DNA आमाबाबुबाट पाउँछौं – त्यसैले हामी उनीहरू जस्तै देखिन्छौं!","DNA मा परिवर्तन – उत्परिवर्तन – नयाँ विशेषताहरू सिर्जना गर्न र विकासमा मद्दत गर्न सक्छ!"]'),

(4032, 3, '🧬', 'Evolution', 'विकास', 'Change Over Time', 'समयसँगै परिवर्तन', 'Science', 'विज्ञान',
'["Natural selection means creatures with useful traits survive and have babies!","Adaptations help animals live better in their homes – like polar bears\' white fur!","Fossils are like time capsules – they show us ancient life!","All life on Earth is related – we all share a common ancestor, way back!"]',
'["प्राकृतिक चयन भनेको उपयोगी विशेषताहरू भएका प्राणीहरू बाँच्छन् र बच्चा जन्माउँछन्!","अनुकूलनले जनावरहरूलाई आफ्नो घरमा राम्रोसँग बाँच्न मद्दत गर्छ – जस्तै ध्रुवीय भालुको सेतो फर!","जीवाश्महरू समय क्याप्सूल जस्तै हुन् – तिनीहरूले हामीलाई प्राचीन जीवन देखाउँछन्!","पृथ्वीमा सबै जीवन सम्बन्धित छ – हामी सबैको एउटै साझा पूर्वज छ, धेरै पहिले!"]'),

(4033, 3, '🌍', 'Biodiversity', 'जैविक विविधता', 'Variety of Life', 'जीवनको विविधता', 'Nature', 'प्रकृति',
'["Biodiversity means all the different kinds of life on Earth – from tiny bugs to giant whales!","There are millions of species – each one is unique!","Biodiversity keeps ecosystems healthy – like a team with different players!","When we lose species, biodiversity drops – and that\'s bad for nature.","We can help by protecting forests, oceans, and all animals!"]',
...
(4034, 3, '🌡️', 'Climate Change', 'जलवायु परिवर्तन', 'Our Warming Planet', 'हाम्रो तातो हुँदै गएको ग्रह', 'Nature', 'प्रकृति',
'["Climate change means Earth is getting warmer over time.","It\'s caused by gases like CO₂ from cars, factories, and cutting down trees.","Effects include melting ice, rising seas, and wilder weather.","We can help by using clean energy, planting trees, and recycling!"]',
'["जलवायु परिवर्तन भनेको पृथ्वी समयसँगै तातो हुँदै गइरहेको छ।","यो कार, कारखाना र रूख कटानबाट CO₂ जस्ता ग्यासहरूका कारण हुन्छ।","प्रभावहरूमा बरफ पग्लनु, समुद्रको सतह बढ्नु र मौसम अझ खराब हुनु समावेश छ।","हामी स्वच्छ ऊर्जा प्रयोग गरेर, रूख रोपेर र रिसाइकल गरेर मद्दत गर्न सक्छौं!"]'),

(4035, 3, '🔬', 'Cell Biology', 'कोशिका जीवविज्ञान', 'The Building Blocks of Life', 'जीवनका निर्माण ईंटहरू', 'Science', 'विज्ञान',
'["Cells are the smallest unit of life – every living thing is made of cells!","Some cells have a nucleus (eukaryotic) and some don\'t (prokaryotic).","Cells have parts called organelles – like a nucleus, mitochondria, and ribosomes!","Cells divide to make new cells – that\'s how we grow and heal!"]',
'["कोशिकाहरू जीवनको सबैभन्दा सानो एकाइ हुन् – प्रत्येक जीवित चीज कोशिकाहरूले बनेको हुन्छ!","कतिपय कोशिकाहरूमा न्यूक्लियस (युकेरियोटिक) हुन्छ र कतिपयमा हुँदैन (प्रोकेरियोटिक)।","कोशिकाहरूमा अंगहरू हुन्छन् – जस्तै न्यूक्लियस, माइटोकोन्ड्रिया र राइबोसोम!","कोशिकाहरू विभाजित भएर नयाँ कोशिकाहरू बनाउँछन् – त्यसरी हामी बढ्छौं र निको हुन्छौं!"]'),

(4036, 3, '🌿', 'Ecological Succession', 'पारिस्थितिक उत्तराधिकार', 'Nature\'s Rebuilding', 'प्रकृतिको पुन: निर्माण', 'Nature', 'प्रकृति',
'["Primary succession – new land forms like islands from volcanoes!","Secondary succession – land recovers after a fire or flood.","First come pioneer species – like lichens and mosses – they prepare the soil.","Over time, a stable ecosystem called a climax community develops.","Nature always finds a way to rebuild!"]',
'["प्राथमिक उत्तराधिकार – ज्वालामुखीबाट टापुहरू जस्तै नयाँ जमिन बन्छ!","द्वितीयक उत्तराधिकार – आगो वा बाढीपछि जमिन पुन: प्राप्त हुन्छ।","पहिले अग्रणी प्रजातिहरू आउँछन् – जस्तै लाइकेन र मस – तिनीहरूले माटो तयार गर्छन्।","समयसँगै, क्लाइम्याक्स समुदाय भनिने स्थिर पारिस्थितिकी तंत्र विकास हुन्छ।","प्रकृतिले सधैं पुन: निर्माण गर्ने तरिका खोज्छ!"]');
